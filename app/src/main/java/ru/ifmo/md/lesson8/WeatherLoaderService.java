...
public class WeatherLoaderService extends IntentService {
    ...
    public static int getIntervalIndex(Context context) {
        ...
        if (interval == INTERVAL_HOUR) {
            return 1;
        } else if (interval == INTERVAL_TWO_HOURS) {
            return 2;
        } else if (interval == INTERVAL_SIX_HOURS) {
            return 3;
        } else if (interval == INTERVAL_TWELVE_HOURS) {
            return 4;
        }
        return 0;
    }
    ...
    private WeatherInfo searchCity(String cityName) throws IOException, SAXException {
        ...
        return WeatherParser.parseCurrent(getReader(queryUrl));
    }
    ...
    private static class WeatherParser {
        ...
        static {
            ...
            city.setStartElementListener(new StartElementListener() {
                @Override
                public void start(Attributes attributes) {
                    sWeather.setCityId(Integer.parseInt(getAttrValue(attributes, "id")));
