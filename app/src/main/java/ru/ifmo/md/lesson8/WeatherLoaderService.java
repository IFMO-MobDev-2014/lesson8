...
public class WeatherLoaderService extends IntentService {
    ...
    public static void startActionAddNewCity(Context context, double first_coordinate, double second_coordinate) {
        Intent intent = new Intent(context, WeatherLoaderService.class);
        intent.setAction(ACTION_ADD_CITY_BY_COORD);
        intent.putExtra(EXTRA_CITY_LATITUDE, first_coordinate);
        intent.putExtra(EXTRA_CITY_LONGITUDE, second_coordinate);
        context.startService(intent);
    }
